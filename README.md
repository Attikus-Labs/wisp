...
disk. Your *whole* history is searchable — secrets included — and like everything in
Wisp it lives only in RAM and the search HUD is excluded from screen capture.
...
- **Secure by design:**
  - **Records everything — leaks nothing.** Wisp keeps a *complete* history, secrets
    included, so passwords and one-time codes from 1Password, Bitwarden, KeePassXC,
    etc. are there when you need them. Safety comes from **containment, not omission**:
    memory-only, no disk, no network, output-only (no keylogging), and the bezel is
    excluded from screen capture — plus an optional post-paste clipboard auto-clear. The
    full threat model — including the residual risks (the system pasteboard is shared;
...
(which arrow walks back to previous copies), launch at login, and the Auto-Paste
Permission (Accessibility) status.
...
Sources/
  WispCore/                 ← all logic + UI (unit-tested library)
    ...
    PrivacyFilter.swift     ← records everything; skips only Wisp's own paste echo
